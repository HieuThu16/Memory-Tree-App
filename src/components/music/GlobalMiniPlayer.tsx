...
export default function GlobalMiniPlayer() {
    ...
    const handleEnded = () => {
      ...
      if (mode === "off") {
        ...
        if (state.currentTrackIndex >= state.playlistTracks.length - 1) {
          useMusicStore.setState({ isPlaying: false, playingUrl: null, currentTrackId: null });
          return;
        }
      }
  ...
  return (
    <div className="fixed bottom-[max(env(safe-area-inset-bottom),16px)] sm:bottom-6 left-1/2 -translate-x-1/2 z-[80] w-[calc(100%-24px)] max-w-sm">
      <div className="glass-card flex items-center justify-between gap-3 p-3 px-4 rounded-full shadow-[0_12px_24px_-8px_rgba(84,67,148,0.3)] bg-white/90">
        ...
        <div className="flex shrink-0 items-center justify-end gap-1.5 opacity-90">
          ...
          <button
            onClick={() => useMusicStore.setState({ playingUrl: null, currentTrackId: null })}
