...
export default function RoomCountdownBoard({
  ...
  const handleUpdate = () => {
    ...
    if (!editTargetDate) {
      addToast("Vui lòng chọn ngày mục tiêu.", "error");
      return;
    }
    const parsedDate = new Date(editTargetDate);
    if (isNaN(parsedDate.getTime())) {
      addToast("Ngày mục tiêu không hợp lệ.", "error");
      return;
    }
    ...
    <div className="space-y-3 rounded-2xl bg-white/80 p-3 sm:p-4">
      ...
      <div className="flex items-center justify-between gap-2 px-1">
        <p className="text-xs font-semibold text-text-secondary">
          {filteredCountdowns.length} / {countdowns.length} sự kiện
        </p>
        <button
          type="button"
          ...
          <div className="w-full max-w-md rounded-2xl border border-border bg-white shadow-xl">
            ...
            <div className="p-4 grid gap-3">
              <input
                ...
                placeholder="Tên sự kiện (VD: Kỉ niệm 1 năm)..."
                disabled={isPending}
      ...
      <div className="rounded-2xl border border-border bg-white/85 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="inline-flex flex-1 justify-end rounded-full bg-transparent p-1">
